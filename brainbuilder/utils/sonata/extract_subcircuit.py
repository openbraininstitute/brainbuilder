...
def _save_sonata_nodes(nodes_path: str | Path, df: pd.DataFrame, population_name: str):
...
def _h5_get_read_chunk_size():
    """Get the desired HDF5 read chunk size from the environment, or use the default."""
...
def _write_edges(
    ...
    with h5py.File(edges_path, "r") as h5in:
        ...
        for src_node_pop, dst_node_pop in it.product(id_mapping, id_mapping):
            ...
            edge_file_name = str(Path(output) / _get_edge_file_name(edge_pop_name))
...
def _write_circuit_config(output, split_nodes):
    ...
    for src, dst in it.product(split_nodes, split_nodes):
        ...
        if (Path(output) / _get_edge_file_name(new_pop_name)).exists():
...
def _write_mapping(output, parent_circ, id_mapping, node_pop_name_mapping):
    ...
    if "id_mapping" in provenance:
        ...
        parent_mapping = utils.load_json(
            Path(parent_circ._circuit_config_path).parent / provenance["id_mapping"]
        )
...
def _copy_pop_morphologies(
    ...
    for _morph_ext, _src_dir in src_morph_dirs.items():
        if _morph_ext == "h5" and Path(_src_dir).is_file():
            ...
            Path(dest_morph_dirs[_morph_ext]).parent.mkdir(parents=True, exist_ok=True)
