...
def generate_h5_updates(h5_morph_path):
    ...
    for file_ in glob.glob(os.path.join(h5_morph_path, '*.h5')):
        ...
        with h5py.File(file_, 'r') as h5:
            ...
            if new_parents:
                ret[os.path.basename(file_)] = {'new_parents': new_parents,
...
def _apply_to_edges(node_ids, updates, edges):
    ...
    for direction in ('afferent', 'efferent', ):
        ...
        if section_name in group and segment_name in group:
            ...
            new_ids = _update_section_and_segment_ids(group[section_name][mask],
                                                      group[segment_name][mask],
                                                      updates)
...
def apply_edge_updates(morphologies, edge_path, h5_updates, population):
    ...
    with h5py.File(edge_path, 'r+') as h5:
        for morphology, node_ids in morphologies.groupby(morphologies):
            ...
            L.debug('apply_edge_updates for morph: %s[%d]: %s',
                    morphology, len(node_ids), node_ids)
            _apply_to_edges(node_ids,
                            h5_updates[morphology + '.h5'],
                            h5['edges'][population])
...
def _morph_section_lengths(morph_path, morph_name):
    ...
    m = morphio.Morphology(os.path.join(morph_path, morph_name + '.h5'),
                           morphio.Option.nrn_order)
    dfs = [pd.DataFrame.from_dict({'segment_id': [0, ],
                                   'section_id': 0,
                                   'cumulative_length': 0.,
                                   'section_length': 1.}), ]
    ...
    return (pd.concat(dfs, sort=False, ignore_index=True)
            .set_index(['section_id', 'segment_id']))
...
def _calculate_section_position(section_lengths, section_ids, segment_ids, offsets):
    ...
    df = pd.DataFrame.from_dict({'section_id': section_ids,
...
def _update_pos(h5_population, section_lengths, ids):
    ...
    for direction in ('afferent', 'efferent', ):
        ...
        mask = _get_synapse_mask_for_ids(h5_population['indices/' + index],
                                         ids.index.to_list(),
...
def write_sonata_pos(morph_path, morphologies, population, edges):
    ...
    def create_section_pos(h5_group, name):
        if not (name + '_section_id' in h5_group and
                name + '_segment_id' in h5_group and
                name + '_segment_offset' in h5_group):
            return
