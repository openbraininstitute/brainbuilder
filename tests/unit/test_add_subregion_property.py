...
def test_add_subregion_property_consistent_layer():
    ...
    lookup_input = np.array([[ 8.09745916,  4.87049681,  8.90753754],
    ...
    region_map = pd.DataFrame(data={'acronym': pd.Series(["plf", "IF", "im", "6b"], index=[11, 12, 14, 16])})
...
def test_add_subregion_property_layer_inconsistency():
    cells = pd.DataFrame(data={
        "layer": ["im", "im", "IF", "plf"], 
        "x": [8.09745916, 24.39533068, 16.80548999, 17.63244536], 
        "y": [4.87049681, 24.92992631, 46.92833477, 52.18625258], 
