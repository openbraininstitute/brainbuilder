...
def make_edges(edge_file_name, edges):
    ...
    with h5py.File(edge_file_name, 'w') as h5:
        for e in edges:
            ...
            add_data(h5, f'/edges/{pop_name}/0/delay', data=np.array([0.5] * len(e.tgid), dtype=float))
            add_data(h5, f'/edges/{pop_name}/edge_type_id', data=np.array([-1] * len(e.tgid), dtype=int))
