...
def test__write_edges(tmp_path, id_mapping, h5_read_chunk_size, expected_dir):
    ...
    split_population._write_edges(
        tmp_path, edges_path, id_mapping, expect_to_use_all_edges=True, h5_read_chunk_size=h5_read_chunk_size)
...
def test_split_population(tmp_path):
    ...
    utils.assert_json_files_equal(
        tmp_path / 'circuit_config.json',
        expected_dir / 'circuit_config.json'
    )
...
def test_split_subcircuit_with_virtual(tmp_path):
    ...
    # edges
    virtual_pop = _find_populations_by_path(networks, 'edges', '$BASE_DIR/edges/virtual_edges_V1.h5')
    assert virtual_pop == {'V1__A': {'type': 'chemical'}, 'V1__B': {'type': 'chemical'}}

    virtual_pop = _find_populations_by_path(networks, 'edges', '$BASE_DIR/V2__C/virtual_edges_V2.h5')
